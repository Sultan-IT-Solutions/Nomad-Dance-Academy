...
async def get_me(user: dict = Depends(require_student)):
    ...
    row = await pool.fetchrow(
        """
        SELECT id, user_id, name, email, phone_number, comment, trial_used, trials_allowed, trials_used, subscription_until
...
async def get_my_attendance(user: dict = Depends(require_student)):
    ...
    attendance_stats = []
    for r in stats_rows:
        total_lessons = r["total_lessons"] if r["total_lessons"] else 1
        max_possible_points = total_lessons * 2
        total_points = float(r["total_points"])

        percentage = round((total_points / max_possible_points) * 100) if max_possible_points > 0 else 0
        ...
        attendance_stats.append({
            ...
            "total": total_lessons,
            "attended": r["present_count"],
            "excused": r["excused_count"],
            "late": r["late_count"],
            "missed": r["absent_count"],
            "percentage": percentage,
            "points": total_points,
            "maxPoints": float(max_possible_points)
        })
