...
export default function GroupDetailPage() {
  ...
  const fetchGroupDetails = async () => {
    try {
      ...
      const formattedGroup: Group = {
        ...
        schedule: groupData.schedule || "Расписание не назначено"
      }
  ...
  const handleAddStudent = async () => {
    ...
    try {
      await API.admin.addStudentToGroup(groupId, selectedStudentId)
  ...
  const handleAddSchedule = async () => {
    ...
    try {
      const scheduleData = {
        ...
        repeat_until: scheduleForm.repeat_enabled ? scheduleForm.repeat_until : null
      }

      await fetch(`http://localhost:8000/admin/groups/${groupId}/lessons`, {
        method: 'POST',
        headers: {
      ...
      setScheduleForm({ date: '', start_time: '', end_time: '', repeat_enabled: false, repeat_frequency: 'weekly', repeat_until: '' })
      fetchGroupDetails()
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
                ...
                {isEditing ? (
                  <>
                    ...
                    <div>
                      ...
                      <Select value={editForm.teacher_id || "none"} onValueChange={(value) => setEditForm({...editForm, teacher_id: value === "none" ? "" : value})}>
                        <SelectTrigger>
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-0" align="start">
                          <CalendarComponent
                            ...
                            onSelect={(date) => {
                              if (date) {
                                setEditForm({...editForm, start_date: date.toISOString().split('T')[0]})
                              }
                            }}
                            locale={ru}
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-0" align="start">
                          <CalendarComponent
                            ...
                            onSelect={(date) => {
                              if (date) {
                                setEditForm({...editForm, end_date: date.toISOString().split('T')[0]})
                              } else {
            ...
            <Card>
              ...
              <CardContent>
                {!group.schedule || group.schedule === "Расписание не назначено" ? (
                  <p className="text-sm text-gray-500">Расписание не назначено</p>
                ) : (
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Mark attendance for lessons. Present=2/2, Excused=2/2, Late=1/2, Absent=0/2 points.
                </p>
              </CardHeader>
              <CardContent>
                <AttendanceManager key={attendanceRefreshKey} groupId={groupId} />
              </CardContent>
            </Card>
          </div>
        </div>
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <CalendarComponent
                    ...
                    onSelect={(date) => {
                      if (date) {
                        setScheduleForm({...scheduleForm, date: date.toISOString().split('T')[0]})
                      }
                    }}
                    locale={ru}
                    disabled={(date) => {
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <CalendarComponent
                          ...
                          onSelect={(date) => {
                            if (date) {
                              setScheduleForm({...scheduleForm, repeat_until: date.toISOString().split('T')[0]})
                            } else {
