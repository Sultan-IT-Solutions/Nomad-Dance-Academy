...
export default function AttendanceManager({ groupId }: AttendanceManagerProps) {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/admin/groups/${groupId}/lessons/${selectedLessonId}/attendance`, {
        method: 'POST',
        headers: {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      case 'P': return <Badge variant="default" className="bg-green-600">Present</Badge>
      case 'E': return <Badge variant="default" className="bg-blue-600">Excused</Badge>
      case 'L': return <Badge variant="default" className="bg-yellow-600">Late</Badge>
      case 'A': return <Badge variant="destructive">Absent</Badge>
  ...
  if (lessons.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">No lessons found for this group.</p>
      </div>
    )
  }
          ...
          return (
            <div key={lesson.id} className="space-y-4">
              ...
              {editing === lesson.id && (
                <Card>
                  <CardHeader>
                    <CardTitle>Edit Lesson - {lesson.class_name}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="class_name">Lesson Name</Label>
                          <Input
                            id="class_name"
              ...
              {selectedLessonId === lesson.id && (
                <Card>
                  <CardHeader>
                    <CardTitle>Mark Attendance - {lesson.class_name}</CardTitle>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="text-sm text-muted-foreground">
                        <strong>Scoring:</strong> Present = 2/2 points, Excused = 2/2 points, Late = 1/2 points, Absent = 0/2 points
                      </div>
                                ...
                                <div className="flex gap-2">
                                  {(['P', 'E', 'L', 'A'] as const).map((status) => (
                                    <Button
                                      ...
                                      title={
                                        status === 'P' ? 'Present (2/2)' :
                                        status === 'E' ? 'Excused (2/2)' :
                                        status === 'L' ? 'Late (1/2)' :
                                        'Absent (0/2)'
                                      }
                                    >
                      ...
                      <div className="flex gap-3 pt-4">
                        <Button onClick={handleSaveAttendance} disabled={saving || Object.keys(attendanceData).length === 0}>
